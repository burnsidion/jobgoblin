...
const imageClass = computed(() =>
  cn('object-cover rounded-lg transition-transform duration-300', props.imageClass),
);
...
const overlayClass = computed(
  () =>
    `absolute inset-0 z-10 bg-gradient-to-t from-black/70 via-black/50 to-transparent transition-opacity duration-300 ${
      direction.value === 'top'
        ? '-translate-y-full'
        : direction.value === 'bottom'
          ? 'translate-y-full'
